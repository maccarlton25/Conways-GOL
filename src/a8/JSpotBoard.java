...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	public JSpotBoard(int width, int height, boolean explode) {
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				Color bg = ((x+y)%2 == 0) ? DEFAULT_BACKGROUND_LIGHT : DEFAULT_BACKGROUND_DARK;
				_spots[x][y] = new JSpot(Color.LIGHT_GRAY, DEFAULT_SPOT_COLOR, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
				((JSpot)_spots[x][y]).setPreferredSize(preferred_size);
				add(((JSpot) _spots[x][y]));
			}			
		}
	}
	public JSpotBoard(int width, int height, String explode) {
		if (width < 1 || height < 1 || width > 50 || height > 50) {
			throw new IllegalArgumentException("Illegal spot board geometry");
		}
		setLayout(new GridLayout(height, width));
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				if(x%2==0) {
					...
					_spots[x][y] = new JSpot(Color.DARK_GRAY, DEFAULT_SPOT_COLOR, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
				}
				((JSpot)_spots[x][y]).setPreferredSize(preferred_size);
				add(((JSpot) _spots[x][y]));
			}			
	...
	public JSpotBoard(int width, int height, int i) {
		if (width < 1 || height < 1 || width > 50 || height > 50) {
			throw new IllegalArgumentException("Illegal spot board geometry");
		}
		setLayout(new GridLayout(height, width));
